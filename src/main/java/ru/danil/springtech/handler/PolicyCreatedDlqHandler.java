...
public class PolicyCreatedDlqHandler {
    ...
    public void handle(RetryableTaskDTO retryableTaskDTO, Acknowledgment ack) {
        try {
            ...
            retryableTaskService.updateStatusByIdWithoutLease(retryableTaskDTO.getId(), RetryableTaskStatus.DELETE_ACTOR_COMPENSATED, RetryableTaskStatus.SEND_TO_KAFKA);
            ack.acknowledge();
        } catch (Exception e) {
            retryableTaskService.updateStatusByIdWithoutLease(retryableTaskDTO.getId(), RetryableTaskStatus.FAILED, RetryableTaskStatus.SEND_TO_KAFKA);
